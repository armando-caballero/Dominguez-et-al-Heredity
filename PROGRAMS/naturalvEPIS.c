...
/* naturalvEPIS.c (modelo de viabilidad) BOTTLENECK N=10,100 generations t=10-20-100*/

/* *********************************************** */

#include "libhdr"
#include "ranlib.h"
#define NN 5001  /* max number of (gen/blocks) */
#define MM 5001  /* max number of NIND */
...
//EPISTASIS
int episK, episType, num_homo_whole, num_homo[MM], homo[MM][31];
double sEPISmin;
...
FILE *fptr, *fgen, *fdat, *fpop, *fopen();
...
getinputs()
{
	tracestart();
	getseed();
	getintandskip("Number of individuals (max 5000) :",&NIND,2,5000);
...
headings()
{
	...
	fprintf(fgen,"\n\n N=%d    epis=%d    episType=%d    PS=%4.2f    L=%f   N.S-LOCI=%d    N.N-LOCI=%d    gens=%d\n Lambda_a=%6.4f   Lambda_L=%6.4f   ave_aL=%6.4f    Psi=%6.4f   threshold=%6.4f   rho=%6.4f   Vs=%6.4f \n ave_s=%6.4f   PP_s=%6.4f   beta_s=%6.4f   alpha_s=%6.4f  dom=%d   k_s=%6.4f   ave_hs=%6.4f\n ave_a=%6.4f   PP_a=%6.4f   beta_a=%6.4f   alpha_a=%6.4f   k_a=%6.4f   ave_ha=%6.4f\n", NIND, episK, episType, PS, L, TOTLOCI-NCRO, NCRO, generations, Lambda_a, Lambda_L, ave_aL, Psi, threshold, rho, Vs, ave_s, P_s, beta_s, alpha_s, dom, k_s, ave_hs, ave_a, P_a, beta_a, alpha_a, k_a, ave_ha);
    	fclose(fgen);
}
	...
	for (i=0; i<NIND; i++)
	{
		...
		for (k=0; k<NCRO; k++)
		{
			...
			for (l=1; l<NLOCI; l++)
			{
				if (homo[k][l] == 1)
				{
					if (episK==2)		genvalm_s[i] *= pow( (1.0 + s[k][l]) , (double)episK-1.0);
					else if (episK==3)	genvalm_s[i] *= pow( (1.0 + s[k][l]) , (double)homo[k][l]-1.0);

					if (tracelevel!=0)	if ((i<=10)&&(k==10)&&(l<=10))    fprintf(fptr,"fec i=%d k=%d l=%d s[k][l]=%f num_homo=%d\n",
								i, k, l, s[k][l], num_homo[k]);
				}
			}
		}

		if (tracelevel!=0)	if (i<=10)    fprintf(fptr," %d    genvalm_s = %f\n", i, genvalm_s[i]);

		if (genvalm_s[i]<0)	genvalm_s[i]=0;
	}
}
	...
	for (i=0; i<NIND; i++)
	{
		...
		for (k=0; k<NCRO; k++)
		{
			for (l=1; l<NLOCI; l++)
			{
		    		...
		    		if (((gm[i][k][0] & RM[l])==RM[l])&&((gm[i][k][1] & RM[l])==RM[l])) 
				{
					homo[k][l] = 1;
					num_homo_whole ++;
					if (tracelevel!=0)	fprintf(fptr,"epis i=%d k=%d l=%d ini=%d num_homo_whole=%d\n",
								i, k, l, initialgen[k][l], num_homo_whole);
				}
			}
		}
		...
		if (num_homo_whole >= 2)
		{
			...
			for (l=1; l<NLOCI; l++)
			{
				...
				{
					if (episK==2)		genvalm_s[i] *= pow( (1.0 + s[k][l]) , (double)episK-1.0);
					else if (episK==3)	genvalm_s[i] *= pow( (1.0 + s[k][l]) , (double)num_homo_whole-1.0);
				}
			}
		}
		...
		if (tracelevel!=0)	fprintf(fptr," %d    genvalm_s = %f\n", i, genvalm_s[i]);
	}
}
...
{ 
    ...
    fprintf(fgen,"\n\n\n gen=%d  N_SO=%4.2f+-%4.2f     SK2=%4.2f+-%4.2f    N_Recom=%4.2f     episK=%d\n", gen+1, accmean(&num_SO[gen/block]), se(&num_SO[gen/block]), accmean(&SK2[gen/block]), se(&SK2[gen/block]), (double)numberofrecom[gen/block]/(2.0*(double)NIND*(double)block), episK);
